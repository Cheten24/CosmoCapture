...
def check_weather_safety(latitude="-37.8136", longitude="144.9631"):
    ...
    try:
        ...
        if humidity is not None and humidity > 90:
            unsafe_reasons.append(f"humidity is too high ({humidity}%)")

        if cloud_cover is not None and cloud_cover > 80:
            unsafe_reasons.append(f"cloud cover is too high ({cloud_cover}%)")

        if precipitation is not None and precipitation > 0:
            unsafe_reasons.append(f"precipitation detected ({precipitation} mm)")

        if wind_speed is not None and wind_speed > 35:
            unsafe_reasons.append(f"wind speed is too high ({wind_speed} km/h)")

        if temperature is not None and (temperature < -5 or temperature > 40):
            unsafe_reasons.append(f"temperature is outside safe range ({temperature}°C)")
        ...
        return {
            "safe": True,
            "reason": (
                f"Weather is safe for booking. "
                f"Cloud cover: {cloud_cover}%, humidity: {humidity}%, "
                f"wind speed: {wind_speed} km/h, precipitation: {precipitation} mm."
            ),
            **weather_data
        }
        ...
        return {
            "safe": False,
            "reason": f"Could not verify live weather data from Open-Meteo: {str(e)}",
            "provider": "Open-Meteo"
        }
...
def check_object_visibility(object_name, date, time, latitude, longitude):
    ...
    if object_name.lower() not in [obj.lower() for obj in visible_objects]:
        return {
            "visible": False,
            "reason": (
                f"{object_name} is not visible at {time} for the selected location. "
                f"Available objects are: {', '.join(visible_objects)}."
            ),
...
def available_objects():
    ...
    return jsonify({
        "status": "success",
        "message": "Available objects loaded successfully.",
        "date": date,
        "time": time,
        "latitude": latitude,
        "longitude": longitude,
...
def create_booking():
    ...
    if not object_name:
        return jsonify({"status": "error", "message": "Please select a space object."}), 400
    ...
    if booking_datetime < datetime.now():
        return jsonify({
            "status": "rejected",
            "message": "Booking rejected: you cannot book a past date or time.",
            "reason": "Selected booking time is already in the past."
        }), 400
    ...
    if not weather_check["safe"]:
        return jsonify({
            "status": "rejected",
            "message": f"Booking rejected: {weather_check['reason']}",
    ...
    if not visibility_check["visible"]:
        return jsonify({
            "status": "rejected",
            "message": f"Booking rejected: {visibility_check['reason']}",
    ...
    if len(same_slot) == 0:
        ...
        queue_position = 0
        message = (
            f"Booking confirmed for {date} at {time} to observe {object_name}. "
            f"{weather_check['reason']}"
        )
    else:
        status = "queued"
        queue_position = len(same_slot) + 1
        message = (
            f"Slot already booked. You are number {queue_position} in the queue. "
            f"{weather_check['reason']}"
        )
