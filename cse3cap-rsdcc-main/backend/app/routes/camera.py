...
ALPACA_BASE = os.getenv('ALPACA_CAMERA_BASE', 'http://localhost:32323/api/v1/camera/0')
CLIENT_ID = int(os.getenv('CLIENT_ID', '1'))

def alpaca_get(action):
    url = f"{ALPACA_BASE}/{action}"
    try:
        res = requests.get(url, params={"ClientID": CLIENT_ID, "ClientTransactionID": 1}, timeout=5)
        return res.json()
...
def alpaca_put(action, data={}):
    url = f"{ALPACA_BASE}/{action}"
    try:
        payload = {"ClientID": CLIENT_ID, "ClientTransactionID": 1, **data}
        res = requests.put(url, data=payload, timeout=10)
        return res.json()
    except Exception as e:
...
def camera_status():
    ...
    try:
        ...
        if connected.get('ErrorNumber', 0) != 0:
            return jsonify({'connected': False, 'error': connected.get('ErrorMessage')}), 200
...
def connect_camera():
    """Connect to camera"""
    try:
        result = alpaca_put('connected', {'Connected': True})
        if result.get('ErrorNumber', 0) != 0:
            return jsonify({'success': False, 'error': result.get('ErrorMessage')}), 400
...
def disconnect_camera():
    """Disconnect camera"""
    try:
        result = alpaca_put('connected', {'Connected': False})
        if result.get('ErrorNumber', 0) != 0:
            return jsonify({'success': False, 'error': result.get('ErrorMessage')}), 400
...
def expose():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': f'Exposure started for {duration}s'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def image_ready():
    ...
    try:
        ...
        return jsonify({'ready': result.get('Value', False)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_image():
    ...
    try:
        ...
        if not ready.get('Value', False):
            return jsonify({'success': False, 'error': 'Image not ready yet'}), 400
...
def abort_exposure():
    ...
    try:
        ...
        if result.get('ErrorNumber', 0) != 0:
            return jsonify({'success': False, 'error': result.get('ErrorMessage')}), 400
        return jsonify({'success': True, 'message': 'Exposure aborted'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
