...
export default function SnapshotCapture({ videoRef, selectedObjectName, onCaptureSuccess }: Props) {
  ...
  const capture = async () => {
    ...
    try {
      ...
      let status = null
      try { status = await apiService.getTelescopeStatus() } catch (e) { console.warn(e) }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        {lastDownloadUrl && (
          <a href={lastDownloadUrl} download className="text-sm underline text-slate-300 hover:text-slate-100 transition-colors">Download last capture</a>
        )}
      </div>
      {message && (
